...
def authentication(request):
    if request.method == 'POST':
        ...
        # print(aadhar_no)--->checking for aadhar_no
...
def get_parties(request):
    ...
    if request.session['email-verified']:
        private_key, public_key = generate_keys()
        print("Generated private_key:", private_key)

        # Use the same key name in the session
...
def create_vote(request):
    ...
    private_key = request.session.get(
        'private_key')  # Use the correct key name
    ...
    print("Selected Party ID:", selected_party_id)
    print("Ballot:", ballot)
    # ballot = f'{uuid}|{selected_party_id}|{curr.timestamp()}'
